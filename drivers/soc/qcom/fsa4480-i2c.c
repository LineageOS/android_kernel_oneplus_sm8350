...
static void fsa4480_reset_and_check_switch(struct fsa4480_priv *fsa_priv)
{
	struct device *dev;
	int retry_cnt = 3;
	...
	do
	{
		...
		dev_info(dev, "%s: reg[0x%x]=0x%x.\n", __func__, FSA4480_SWITCH_STATUS0, switch_status0);
		regmap_read(fsa_priv->regmap, FSA4480_SWITCH_STATUS1, &switch_status1);
		dev_info(dev, "%s: reg[0x%x]=0x%x.\n", __func__, FSA4480_SWITCH_STATUS1, switch_status1);

		if ((switch_status0 & 0x3F) != 0x5 || (switch_status1 & 0x3F) != 0x0) {
			dev_err(dev, "%s: switch reset failed!!\n", __func__);
			usleep_range(1000, 1005);
		} else {
			break;
		}
