...
static void init_project_version(void)
{
	...
	if (is_new_cdt()) {
		...
		if (oplus_info_temp) {
			remove_proc_entry("oplusVersion_tmp/operatorName",
					  NULL);
			pr_err("remove proc operatorName\n");
...
static void update_manifest(struct proc_dir_entry *parent_1,
			    struct proc_dir_entry *parent_2)
{
	...
	if (parent_1 && parent_2) {
		if (sim_card_num != NULL) {
			if (sim_card_num[0] == '0') {
				proc_symlink("manifest", parent_1,
					     manifest_src[0]); //single sim
				proc_symlink("manifest", parent_2,
					     manifest_src[0]);
			} else {
				proc_symlink("manifest", parent_1,
					     manifest_src[1]);
				      ...
				      struct proc_dir_entry *parent_2)
{
	...
	if (parent_1 && parent_2) {
		if (sim_card_num != NULL) {
			if (sim_card_num[0] == '0') {
				proc_symlink("telephony_manifest", parent_1,
					     manifest_src[0]); //single sim
				proc_symlink("telephony_manifest", parent_2,
					     manifest_src[0]);
			} else {
				proc_symlink("telephony_manifest", parent_1,
					     manifest_src[1]);
...
static int __init oplus_project_init(void)
{
	...
	p_entry =
		proc_create_data("prjName", S_IRUGO, oplus_info_temp,
				 &project_info_fops, UINT2Ptr(PROJECT_VERSION));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("pcbVersion", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(PCB_VERSION));
	if (!p_entry)
		goto error_init;

	p_entry =
		proc_create_data("oplusBootmode", S_IRUGO, oplus_info_temp,
				 &project_info_fops, UINT2Ptr(OPLUS_BOOTMODE));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("RFType", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(RF_INFO));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("modemType", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(MODEM_TYPE));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("operatorName", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(OPERATOR_NAME));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("secureType", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(SECURE_TYPE));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("secureStage", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(SECURE_STAGE));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("ocp", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(OCP_NUMBER));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("serialID", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(SERIAL_NUMBER));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("engVersion", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(ENG_VERSION));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("isConfidential", S_IRUGO, oplus_info_temp,
				   &project_info_fops,
				   UINT2Ptr(CONFIDENTIAL_STATUS));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("cdt", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(CDT_INTEGRITY));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("feature", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(OPLUS_FEATURE));
	if (!p_entry)
		goto error_init;

	p_entry = proc_create_data("test", S_IRUGO, oplus_info_temp,
				   &project_info_fops, UINT2Ptr(PROJECT_TEST));
	if (!p_entry)
		goto error_init;
