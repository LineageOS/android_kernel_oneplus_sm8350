...
#if defined(CONFIG_OPLUS_FEATURE_FEEDBACK) ||                                  \
	defined(CONFIG_OPLUS_FEATURE_FEEDBACK_MODULE)
				oplus_kevent_fb(FB_SENSOR, SENSOR_POWER_TYPE,
						payload);
#endif
			}
		}
...
#if defined(CONFIG_OPLUS_FEATURE_FEEDBACK) ||                                  \
	defined(CONFIG_OPLUS_FEATURE_FEEDBACK_MODULE)
		oplus_kevent_fb(FB_SENSOR, g_fb_conf[index].fb_event_id,
				payload);
...
static int sensor_feedback_probe(struct platform_device *pdev)
{
	...
	sensor_fb_cxt->report_task =
		kthread_create(sensor_report_thread, (void *)sensor_fb_cxt,
