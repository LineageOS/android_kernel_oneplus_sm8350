...
static void oplus_short_ic_init_work_func(struct work_struct *work)
{
	...
	if (volt_threshold != OPLUS_SHORT_IC_TEMP_VOLT_DROP_THRESH_VAL) {
		rc = i2c_smbus_write_byte_data(
			chip->client, OPLUS_SHORT_IC_TEMP_VOLT_DROP_THRESH_REG,
		...
		retry_cnt = 0;
		while (rc != OPLUS_SHORT_IC_TEMP_VOLT_DROP_THRESH_VAL &&
		       retry_cnt < 3) {
			usleep_range(5000, 5000 + 100);
			rc = i2c_smbus_read_byte_data(
				chip->client,
				OPLUS_SHORT_IC_TEMP_VOLT_DROP_THRESH_REG);
			retry_cnt++;
		}
...
bool oplus_short_ic_otp_check(void)
{
	...
	if (chip->b_factory_id_get == false || chip->b_volt_drop_set == false ||
	    chip->b_work_mode_set == false) {
				       ...
				       const struct i2c_device_id *id)
{
	...
	short_ic_chip = chip;
	INIT_DELAYED_WORK(&chip->oplus_short_ic_init_work,
			  oplus_short_ic_init_work_func);
	msleep(80);
