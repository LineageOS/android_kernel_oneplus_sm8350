...
int __attribute__((weak)) qpnp_get_prop_charger_voltage_now()
{
	return 0;
}
...
int oplus_usbtemp_dischg_action(struct oplus_chg_chip *chip)
{
	...
	if (get_eng_version() == HIGH_TEMP_AGING) {
		...
		rc = pinctrl_select_state(chip->normalchg_gpio.pinctrl,
					  chip->normalchg_gpio.dischg_disable);
	} else {
		pr_err("[oplus_usbtemp_dischg_action]: set vbus down");
		rc = pinctrl_select_state(chip->normalchg_gpio.pinctrl,
					  chip->normalchg_gpio.dischg_enable);
	}
...
int oplus_usbtemp_monitor_common(void *data)
{
	...
	while (!kthread_should_stop()) {
		if (chip->chg_ops->oplus_usbtemp_monitor_condition != NULL) {
			wait_event_interruptible(chip->oplus_usbtemp_wq,
						 chip->usbtemp_check);
		} else {
		...
		get_usb_temp(chip);
		if ((chip->usb_temp_l < USB_50C) &&
		    (chip->usb_temp_r < USB_50C)) {
		...
		if ((chip->usbtemp_volt_l < USB_50C) &&
		    (chip->usbtemp_volt_r < USB_50C) &&
		...
		if (chip->tbatt_temp / 10 <= USB_50C &&
			...
			if (count_r >= retry_cnt || count_l >= retry_cnt) {
				if (!IS_ERR_OR_NULL(chip->normalchg_gpio
							    .dischg_enable)) {
		...
		if (chip->tbatt_temp / 10 > USB_50C &&
		    (((chip->usb_temp_l >= chip->tbatt_temp / 10 + 7) &&
		      (chip->usb_temp_l < USB_100C)) ||
		     ((chip->usb_temp_r >= chip->tbatt_temp / 10 + 7) &&
		      (chip->usb_temp_r < USB_100C)))) {
			pr_err("in loop 1");
			for (i = 1; i <= retry_cnt; i++) {
				mdelay(RETRY_CNT_DELAY);
			...
			if (((current_temp_l - last_usb_temp_l) >= 3) ||
				...
				for (i = 1; i <= retry_cnt; i++) {
					...
					if ((chip->usb_temp_l -
					    ...
					    chip->usb_temp_l < USB_100C)
						count_l++;
					pr_err("countl : %d,countr : %d",
					       count_l, count_r);
				}
				current_temp_l = chip->usb_temp_l;
				current_temp_r = chip->usb_temp_r;
				if ((count_l >= retry_cnt &&
				     chip->usb_temp_l > USB_30C &&
				     chip->usb_temp_l < USB_100C) ||
				    (count_r >= retry_cnt &&
