...
static int oplus_chg_typec_cc_orientation(struct oplus_chg_device *dev,
					  int *orientation)
{
	struct oplus_chg_ic_dev *typec_ic;
...
static int oplus_get_otg_online_status(struct oplus_chg_device *dev,
				       int *otg_online)
{
	struct oplus_chg_ic_dev *typec_ic;
	struct oplus_chg_ic_typec_ops *typec_ic_ops;
...
{
	struct oplus_chg_device *dev =
		container_of(work, struct oplus_chg_device, otg_enable_work);
...
{
	struct oplus_chg_device *dev =
		container_of(work, struct oplus_chg_device, otg_disable_work);
...
{
	struct oplus_chg_device *dev =
		container_of(work, struct oplus_chg_device, reg_dump_work);
...
static void oplus_chg_batt_update_work(struct work_struct *work)
{
	...
	struct oplus_chg_device *dev =
		container_of(dwork, struct oplus_chg_device, batt_update_work);
	...
	if (dev->batt_update_count < 30) {
		...
		schedule_delayed_work(&dev->batt_update_work,
...
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_device *dev = container_of(
		dwork, struct oplus_chg_device, connect_check_work);
	struct oplus_chg_chip *chip = oplus_chg_get_chg_struct();
...
static void oplus_chg_charger_status_check_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_chg_device *dev = container_of(
		dwork, struct oplus_chg_device, charger_status_check_work);
	struct oplus_chg_chip *chip = oplus_chg_get_chg_struct();
	...
	if (!chip->chg_ops->check_chrdet_status()) {
		if (is_usb_ocm_available(dev))
			oplus_chg_mod_event(dev->usb_ocm, dev->usb_ocm,
...
static ssize_t
oplus_chg_intf_usb_upgrade_fw_store(struct device *dev,
				    struct device_attribute *attr,
...
static enum oplus_chg_mod_property oplus_chg_intf_usb_uevent_props[] = {
	...
	OPLUS_CHG_PROP_USB_TYPE,    OPLUS_CHG_PROP_FASTCHG_STATUS,
	OPLUS_CHG_PROP_ADAPTER_SID, OPLUS_CHG_PROP_CHG_ENABLE,
	OPLUS_CHG_PROP_OTG_MODE,
};
...
static struct oplus_chg_exten_prop oplus_chg_intf_usb_exten_props[] = {
	OPLUS_CHG_EXTEN_RWATTR(OPLUS_CHG_EXTERN_PROP_UPGRADE_FW,
			       oplus_chg_intf_usb_upgrade_fw),
};
#endif

__maybe_unused static const enum oplus_chg_usb_type
	oplus_type_to_oplus_chg_usb_type[] = {
		OPLUS_CHG_USB_TYPE_UNKNOWN, OPLUS_CHG_USB_TYPE_UNKNOWN,
		OPLUS_CHG_USB_TYPE_UNKNOWN, OPLUS_CHG_USB_TYPE_UNKNOWN,
		OPLUS_CHG_USB_TYPE_SDP,	    OPLUS_CHG_USB_TYPE_DCP,
		OPLUS_CHG_USB_TYPE_CDP,	    OPLUS_CHG_USB_TYPE_ACA,
		OPLUS_CHG_USB_TYPE_C,	    OPLUS_CHG_USB_TYPE_PD,
		OPLUS_CHG_USB_TYPE_PD_DRP,  OPLUS_CHG_USB_TYPE_APPLE_BRICK_ID,
	};
				       ...
				       union oplus_chg_mod_propval *pval)
{
	...
	case OPLUS_CHG_PROP_ONLINE:
		...
		if (chip->usb_chg_disable) {
			...
			pval->intval = atomic_read(&oplus_dev->usb_online) &&
				       (chip->charger_exist_delay ?
						1 :
						      chip->charger_exist);
			if (!chip->mmi_chg &&
			    (chip->charger_volt > CHG_VUSBIN_VOL_THR))
				pval->intval = true;
			if (oplus_dev->icon_debounce)
				pval->intval = true;
			if (boot_mode == MSM_BOOT_MODE__CHARGE &&
			    chip->disconnect_vbus == true) {
	...
	case OPLUS_CHG_PROP_USB_TYPE:
		...
		} else if (oplus_warp_get_fast_chg_type() ==
			   CHARGER_SUBTYPE_FASTCHG_SWARP) {
			pval->intval = OPLUS_CHG_USB_TYPE_SWARP;
		} else {
			sid = oplus_warp_get_adapter_sid();
			if (sid == 0) {
				if (atomic_read(&oplus_dev->usb_online) == 1 ||
	...
	case OPLUS_CHG_PROP_CHG_ENABLE:
		...
		if (chip->usb_chg_disable == !!pval->intval) {
			printk_ratelimited(
				KERN_ERR
				"OPLUS_CHG[INTF]: %s[%d]: usb_chg_disable status does not match ibus, usb_chg_disable=%d, ibus=%d\n",
				__func__, __LINE__, chip->usb_chg_disable,
				ibus_ma);
		}
		break;
	...
	case OPLUS_CHG_PROP_REG_DUMP:
		if (oplus_dev->usb_ocm)
			oplus_chg_anon_mod_event(oplus_dev->usb_ocm,
	...
	case OPLUS_CHG_PROP_FAST_CHG_TYPE:
		...
		if (chip->is_oplus_svid) {
			pval->intval = oplus_warp_get_fast_chg_type();
		} else if (!chip->svid_verified &&
			   (pval->intval == CHARGER_SUBTYPE_PD)) {
			pval->intval = CHARGER_SUBTYPE_DEFAULT;
		}
		if (oplus_dev->sid_backup != 0)
			pval->intval = oplus_warp_convert_fast_chg_type(
				sid_to_adapter_id(oplus_dev->sid_backup));
...
#else
		if (pval->intval == 0) {
			if (oplus_wpc_get_adapter_type() ==
				    CHARGER_SUBTYPE_FASTCHG_WARP ||
			    oplus_wpc_get_adapter_type() ==
				    CHARGER_SUBTYPE_FASTCHG_SWARP)
				       ...
				       const union oplus_chg_mod_propval *pval)
{
	...
	case OPLUS_CHG_PROP_OTG_SWITCH:
		chip->otg_switch =
			(bool)pval->intval &&
			chip->chg_ops->otg_set_switch((bool)pval->intval);
...
static int
oplus_chg_intf_usb_prop_is_writeable(struct oplus_chg_mod *ocm,
				     enum oplus_chg_mod_property prop)
{
	switch (prop) {
	case OPLUS_CHG_PROP_VOLTAGE_MAX:
...
{
	struct oplus_chg_device *op_dev =
		container_of(nb, struct oplus_chg_device, usb_event_nb);
	...
	case OPLUS_CHG_EVENT_OFFLINE:
		...
		if (chip && chip->check_abnormal_voltage_work.work.func)
			schedule_delayed_work(
				&chip->check_abnormal_voltage_work,
	...
	case OPLUS_CHG_EVENT_PRESENT:
		...
		if (chip && chip->ctrl_lcm_frequency.work.func)
			mod_delayed_work(system_highpri_wq,
					 &chip->ctrl_lcm_frequency, 50);
		if (chip && chip->recovery_chg_type_work.work.func &&
		    chip->abnormal_volt_detected &&
		    (boot_mode == MSM_BOOT_MODE__NORMAL))
			schedule_delayed_work(&chip->recovery_chg_type_work,
...
{
	struct oplus_chg_device *op_dev =
		container_of(nb, struct oplus_chg_device, usb_mod_nb);
	struct oplus_chg_mod *owner_ocm = v;
	struct oplus_chg_chip *chip = oplus_chg_get_chg_struct();
	int hw_detect = 0;
	...
	case OPLUS_CHG_EVENT_OFFLINE:
		...
		if (oplus_warp_ignore_event() == true) {
			schedule_delayed_work(
				&op_dev->charger_status_check_work,
		...
		if (!strcmp(owner_ocm->desc->name, "usb")) {
			...
			if ((atomic_read(&op_dev->usb_online) != 0) ||
			    (atomic_read(&op_dev->usb_present) != 0)) {
...
{
	struct oplus_chg_device *op_dev =
		container_of(nb, struct oplus_chg_device, usb_changed_nb);
	struct oplus_chg_mod *owner_ocm = v;

	switch (val) {
	case OPLUS_CHG_EVENT_CHANGED:
		if (!strcmp(owner_ocm->desc->name, "usb") &&
		    is_usb_psy_available(op_dev))
			power_supply_changed(op_dev->usb_psy);
		break;
	default:
		break;
...
static ssize_t oplus_chg_intf_batt_voltage_now_cell_show(
...
static struct oplus_chg_exten_prop oplus_chg_intf_batt_exten_props[] = {
	OPLUS_CHG_EXTEN_ROATTR(OPLUS_CHG_EXTERN_PROP_VOLTAGE_NOW_CELL,
			       oplus_chg_intf_batt_voltage_now_cell),
};
					...
					union oplus_chg_mod_propval *pval)
{
	...
	case OPLUS_CHG_PROP_STATUS:
		...
		if (wls_online && is_comm_ocm_available(oplus_dev)) {
			pval->intval = oplus_chg_comm_get_batt_status(
				oplus_dev->comm_ocm);
		} else {
	...
	case OPLUS_CHG_PROP_CHG_ENABLE:
		...
		if (is_wls_ocm_available(oplus_dev)) {
			rc = oplus_chg_mod_get_property(
				oplus_dev->wls_ocm,
				OPLUS_CHG_PROP_BATT_CHG_ENABLE, &temp_val);
			if (rc < 0) {
	...
	case OPLUS_CHG_PROP_CAPACITY:
		if (oplus_dev->soc_debug_mode) {
			...
			if (chip->warp_show_ui_soc_decimal == true &&
				...
				pval->intval = (chip->ui_soc_integer +
						chip->ui_soc_decimal) /
					       1000;
			} else {
	...
	case OPLUS_CHG_PROP_HEALTH:
		...
		if (is_comm_ocm_available(oplus_dev) && wls_online)
			batt_health = oplus_chg_comm_get_batt_health(
				oplus_dev->comm_ocm);
	...
	case OPLUS_CHG_PROP_BATTERY_NOTIFY_CODE:
		...
		pval->intval = batt_err_code_table_color_to_oos(
			chip->notify_code | oplus_dev->notify_code);
#else
					...
					const union oplus_chg_mod_propval *pval)
{
	...
	case OPLUS_CHG_PROP_CHG_ENABLE:
		if (atomic_read(&oplus_dev->usb_online)) {
			rc = oplus_chg_intf_mmi_chg_enable(chip,
							   !!pval->intval);
			if (rc < 0) {
		...
		if (is_wls_ocm_available(oplus_dev)) {
			...
			if (wls_online) {
				rc = oplus_chg_mod_set_property(
					oplus_dev->wls_ocm,
					OPLUS_CHG_PROP_BATT_CHG_ENABLE, pval);
				if (rc < 0) {
...
static int
oplus_chg_intf_batt_prop_is_writeable(struct oplus_chg_mod *ocm,
				      enum oplus_chg_mod_property prop)
{
	switch (prop) {
	case OPLUS_CHG_PROP_VOLTAGE_MAX:
...
{
	struct oplus_chg_device *op_dev =
		container_of(nb, struct oplus_chg_device, batt_event_nb);
	struct oplus_chg_mod *owner_ocm = v;
	struct oplus_chg_chip *chip = oplus_chg_get_chg_struct();
...
{
	struct oplus_chg_device *op_dev =
		container_of(nb, struct oplus_chg_device, batt_changed_nb);
	struct oplus_chg_mod *owner_ocm = v;

	switch (val) {
	case OPLUS_CHG_EVENT_CHANGED:
		if (!strcmp(owner_ocm->desc->name, "battery") &&
		    is_batt_psy_available(op_dev))
			power_supply_changed(op_dev->batt_psy);
		break;
	default:
		break;
...
static int oplus_chg_intf_init_mod(struct oplus_chg_device *oplus_dev)
{
	...
	oplus_dev->usb_ocm = oplus_chg_mod_register(
		oplus_dev->dev, &oplus_chg_intf_usb_mod_desc, &ocm_cfg);
	...
	oplus_dev->usb_mod_nb.notifier_call =
		oplus_chg_intf_usb_mod_notifier_call;
	rc = oplus_chg_reg_mod_notifier(oplus_dev->usb_ocm,
					&oplus_dev->usb_mod_nb);
	if (rc) {
		pr_err("register usb mod notifier error, rc=%d\n", rc);
		goto reg_usb_mod_notifier_err;
	}
	oplus_dev->usb_event_nb.notifier_call =
		oplus_chg_intf_usb_event_notifier_call;
	...
	oplus_dev->usb_changed_nb.notifier_call =
		oplus_chg_intf_usb_changed_notifier_call;
	...
	oplus_dev->batt_ocm = oplus_chg_mod_register(
		oplus_dev->dev, &oplus_chg_intf_batt_mod_desc, &ocm_cfg);
	...
	oplus_dev->batt_mod_nb.notifier_call =
		oplus_chg_intf_batt_mod_notifier_call;
	rc = oplus_chg_reg_mod_notifier(oplus_dev->batt_ocm,
					&oplus_dev->batt_mod_nb);
	if (rc) {
		pr_err("register batt mod notifier error, rc=%d\n", rc);
		goto reg_batt_mod_notifier_err;
	}
	oplus_dev->batt_event_nb.notifier_call =
		oplus_chg_intf_batt_event_notifier_call;
	...
	oplus_dev->batt_changed_nb.notifier_call =
		oplus_chg_intf_batt_changed_notifier_call;
...
#ifndef CONFIG_OPLUS_CHG_OOS
	oplus_dev->ac_ocm = oplus_chg_mod_register(
		oplus_dev->dev, &oplus_chg_intf_ac_mod_desc, &ocm_cfg);
	...
	oplus_dev->ac_mod_nb.notifier_call =
		oplus_chg_intf_ac_mod_notifier_call;
	rc = oplus_chg_reg_mod_notifier(oplus_dev->ac_ocm,
					&oplus_dev->ac_mod_nb);
	if (rc) {
		pr_err("register ac mod notifier error, rc=%d\n", rc);
		goto reg_ac_mod_notifier_err;
	}
	oplus_dev->ac_event_nb.notifier_call =
		oplus_chg_intf_ac_event_notifier_call;
	...
	oplus_dev->ac_changed_nb.notifier_call =
		oplus_chg_intf_ac_changed_notifier_call;
...
reg_batt_event_notifier_err:
	oplus_chg_unreg_mod_notifier(oplus_dev->batt_ocm,
				     &oplus_dev->batt_mod_nb);
...
reg_usb_event_notifier_err:
	oplus_chg_unreg_mod_notifier(oplus_dev->usb_ocm,
				     &oplus_dev->usb_mod_nb);
...
static int oplus_chg_intf_probe(struct platform_device *pdev)
{
	...
	oplus_dev = devm_kzalloc(&pdev->dev, sizeof(struct oplus_chg_device),
				 GFP_KERNEL);
	if (oplus_dev == NULL) {
	...
	of_platform_populate(oplus_dev->dev->of_node, NULL, NULL,
			     oplus_dev->dev);
	oplus_dwc3_set_notifier(&oplus_dwc3_msm_notify_event);

	INIT_DELAYED_WORK(&oplus_dev->batt_update_work,
			  oplus_chg_batt_update_work);
	INIT_DELAYED_WORK(&oplus_dev->connect_check_work,
			  oplus_chg_connect_check_work);
	INIT_DELAYED_WORK(&oplus_dev->charger_status_check_work,
			  oplus_chg_charger_status_check_work);
#ifdef OPLUS_CHG_REG_DUMP_ENABLE
