...
static void oplus_chg_wls_status_keep_clean_work(struct work_struct *work)
{
	...
	struct oplus_gki_device *ogdev = container_of(
		dwork, struct oplus_gki_device, status_keep_clean_work);
	...
	if (!is_wls_ocm_available(ogdev)) {
		...
		if (temp_val.intval == WLS_SK_BY_HAL) {
			...
			oplus_chg_mod_set_property(ogdev->wls_ocm,
						   ...
						   &temp_val);
			schedule_delayed_work(&ogdev->status_keep_clean_work,
			    ...
			    union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		rc = oplus_chg_mod_get_property(
			ogdev->wls_ocm, OPLUS_CHG_PROP_ONLINE_KEEP, &temp_val);
		if (rc < 0)
			temp_val.intval = 0;
		rc = oplus_chg_mod_get_property(
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->wls_ocm, OPLUS_CHG_PROP_VOLTAGE_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		rc = oplus_chg_mod_get_property(
			ogdev->wls_ocm, OPLUS_CHG_PROP_VOLTAGE_MAX,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->wls_ocm, OPLUS_CHG_PROP_CURRENT_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		rc = oplus_chg_mod_get_property(
			ogdev->wls_ocm, OPLUS_CHG_PROP_CURRENT_MAX,
			(union oplus_chg_mod_propval *)pval);
		break;
			    ...
			    union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		rc = oplus_chg_mod_get_property(
			ogdev->usb_ocm, OPLUS_CHG_PROP_PRESENT,
			(union oplus_chg_mod_propval *)pval);
		if (pval->intval == 0) {
			rc = oplus_chg_mod_get_property(
				ogdev->usb_ocm, OPLUS_CHG_PROP_ONLINE,
				(union oplus_chg_mod_propval *)pval);
		}
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->usb_ocm, OPLUS_CHG_PROP_VOLTAGE_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		rc = oplus_chg_mod_get_property(
			ogdev->usb_ocm, OPLUS_CHG_PROP_VOLTAGE_MAX,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->usb_ocm, OPLUS_CHG_PROP_CURRENT_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		rc = oplus_chg_mod_get_property(
			ogdev->usb_ocm, OPLUS_CHG_PROP_CURRENT_MAX,
			(union oplus_chg_mod_propval *)pval);
		break;
	...
	case POWER_SUPPLY_PROP_USB_TYPE:
		rc = oplus_chg_mod_get_property(
			ogdev->usb_ocm, OPLUS_CHG_PROP_USB_TYPE,
			(union oplus_chg_mod_propval *)pval);
		if (rc == 0)
	...
	case POWER_SUPPLY_PROP_TEMP:
		rc = oplus_chg_mod_get_property(
			ogdev->usb_ocm, OPLUS_CHG_PROP_CON_TEMP1,
			(union oplus_chg_mod_propval *)pval);
		break;
	default:
		pr_err("get prop %d is not supported\n", prop);
...
static enum power_supply_property usb_props[] = {
	...
	POWER_SUPPLY_PROP_USB_TYPE,    POWER_SUPPLY_PROP_TEMP,
};
				...
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (is_wls_ocm_available(ogdev)) {
			...
			if (rc == 0)
				wls_online |= !!temp_val.intval;
			rc = oplus_chg_mod_get_property(
				ogdev->wls_ocm, OPLUS_CHG_PROP_STATUS_KEEP,
				&temp_val);
			if (rc == 0)
				wls_status_keep = !!temp_val.intval;
		}
		if (wls_status_keep) {
			...
			rc = oplus_chg_mod_get_property(
				ogdev->batt_ocm, OPLUS_CHG_PROP_STATUS,
				(union oplus_chg_mod_propval *)pval);
			if (rc == 0)
	...
	case POWER_SUPPLY_PROP_HEALTH:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_HEALTH,
			(union oplus_chg_mod_propval *)pval);
		if (rc == 0)
	...
	case POWER_SUPPLY_PROP_PRESENT:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_PRESENT,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CHARGE_TYPE,
			(union oplus_chg_mod_propval *)pval);
		if (rc == 0)
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CAPACITY,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_OCV:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_VOLTAGE_OCV,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_VOLTAGE_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_VOLTAGE_MAX,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CURRENT_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CHARGE_CONTROL_LIMIT,
			(union oplus_chg_mod_propval *)pval);
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_TEMP,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_TECHNOLOGY:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_TECHNOLOGY,
			(union oplus_chg_mod_propval *)pval);
		if (rc == 0)
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CHARGE_COUNTER,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CYCLE_COUNT,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CHARGE_FULL_DESIGN,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CHARGE_FULL,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_MODEL_NAME:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_MODEL_NAME,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_TIME_TO_FULL_AVG:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_TIME_TO_FULL_AVG,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_TIME_TO_FULL_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_TIME_TO_EMPTY_AVG:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_TIME_TO_EMPTY_AVG,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_POWER_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_POWER_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_POWER_AVG:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_POWER_AVG,
			(union oplus_chg_mod_propval *)pval);
		break;
#ifndef CONFIG_OPLUS_CHG_OOS
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_CHARGE_NOW,
			(union oplus_chg_mod_propval *)pval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		rc = oplus_chg_mod_get_property(
			ogdev->batt_ocm, OPLUS_CHG_PROP_VOLTAGE_MIN,
			(union oplus_chg_mod_propval *)pval);
		break;
#endif
	default:
...
static int oplus_chg_gki_init_psy(struct oplus_gki_device *dev)
{
	...
	dev->batt_psy =
		devm_power_supply_register(dev->dev, &batt_psy_desc, &psy_cfg);
	...
	dev->usb_psy =
		devm_power_supply_register(dev->dev, &usb_psy_desc, &psy_cfg);
	...
	if (dev->support_wls) {
		dev->wls_psy = devm_power_supply_register(
			dev->dev, &wls_psy_desc, &psy_cfg);
...
static int oplus_chg_gki_probe(struct platform_device *pdev)
{
	...
	gki_dev = devm_kzalloc(&pdev->dev, sizeof(struct oplus_gki_device),
			       GFP_KERNEL);
	if (gki_dev == NULL) {
	...
	INIT_DELAYED_WORK(&gki_dev->status_keep_clean_work,
			  oplus_chg_wls_status_keep_clean_work);
	gki_dev->status_wake_lock =
		wakeup_source_register(gki_dev->dev, "status_wake_lock");
