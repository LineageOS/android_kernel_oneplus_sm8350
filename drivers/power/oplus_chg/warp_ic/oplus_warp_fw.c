...
bool is_warp_asic_hwid_check_by_i2c(struct oplus_warp_chip *chip)
{
	...
	if (!node) {
		...
		return of_property_read_bool(node,
...
static int opchg_bq27541_gpio_pinctrl_init(struct oplus_warp_chip *chip)
{
	...
	if (1) {
		chip->warp_gpio.gpio_switch1_act_switch2_act =
			pinctrl_lookup_state(chip->warp_gpio.pinctrl,
		...
		chip->warp_gpio.gpio_switch1_ctr1_act =
			pinctrl_lookup_state(chip->warp_gpio.pinctrl,
					     "charging_switch1_ctr1_active");
		if (IS_ERR_OR_NULL(chip->warp_gpio.gpio_switch1_ctr1_act)) {
			chg_err(": %d Failed to get the charging_switch1_ctr1_active handle\n",
				__LINE__);
		}

		chip->warp_gpio.gpio_switch1_ctr1_sleep = pinctrl_lookup_state(
			chip->warp_gpio.pinctrl, "charging_switch1_ctr1_sleep");
	...
	chip->warp_gpio.gpio_switch1_act_switch2_sleep = pinctrl_lookup_state(
		chip->warp_gpio.pinctrl, "switch1_act_switch2_sleep");
	...
	chip->warp_gpio.gpio_switch1_sleep_switch2_act = pinctrl_lookup_state(
		chip->warp_gpio.pinctrl, "switch1_sleep_switch2_act");
...
void oplus_warp_fw_type_dt(struct oplus_warp_chip *chip)
{
	...
	chip->warp_current_lvl_cnt = of_property_count_elems_of_size(
		node, "qcom,warp_current_lvl", sizeof(*chip->warp_current_lvl));
	if (chip->warp_current_lvl_cnt > 0) {
		chg_err("warp_current_lvl_cnt[%d]\n",
			chip->warp_current_lvl_cnt);
		chip->warp_current_lvl =
			devm_kcalloc(chip->dev, chip->warp_current_lvl_cnt,
		...
		rc = of_property_read_u32_array(node, "qcom,warp_current_lvl",
						chip->warp_current_lvl,
						chip->warp_current_lvl_cnt);
		if (rc) {
	...
	chip->support_warp_by_normal_charger_path = of_property_read_bool(
		node, "qcom,support_warp_by_normal_charger_path");
	...
	chip->warp_fw_update_newmethod =
		of_property_read_bool(node, "qcom,warp_fw_update_newmethod");
	...
	rc = of_property_read_u32(node, "qcom,warp-high-temp",
				  &chip->warp_high_temp);
	if (rc) {
	...
	rc = of_property_read_u32(node, "qcom,warp-low-soc",
				  &chip->warp_low_soc);
	if (rc) {
		chip->warp_low_soc = 1;
	} else {
	...
	rc = of_property_read_u32(node, "qcom,warp-high-soc",
				  &chip->warp_high_soc);
	if (rc) {
	...
	chip->warp_multistep_adjust_current_support = of_property_read_bool(
		node, "qcom,warp_multistep_adjust_current_support");
	...
	rc = of_property_read_u32(node,
				  "qcom,warp_strategy2_batt_up_down_temp2",
	...
	rc = of_property_read_u32(node,
				  "qcom,warp_strategy2_batt_up_down_temp4",
...
int oplus_warp_asic_hwid_check(struct oplus_warp_chip *chip)
{
	...
	chip->warp_gpio.warp_asic_id_gpio =
		of_get_named_gpio(node, "qcom,warp_asic_id-gpio", 0);
	if (chip->warp_gpio.warp_asic_id_gpio < 0) {
		...
		if (gpio_is_valid(chip->warp_gpio.warp_asic_id_gpio)) {
			rc = gpio_request(chip->warp_gpio.warp_asic_id_gpio,
					  "warp-asic-id-gpio");
			if (rc) {
	...
	chip->warp_gpio.gpio_warp_asic_id_sleep = pinctrl_lookup_state(
		chip->warp_gpio.pinctrl, "warp_asic_id_sleep");
	...
	chip->warp_gpio.gpio_warp_asic_id_active = pinctrl_lookup_state(
		chip->warp_gpio.pinctrl, "warp_asic_id_active");
...
/*This is only for P60 P(17197 P)*/
	...
	chip->warp_gpio.warp_mcu_id_gpio =
		of_get_named_gpio(node, "qcom,warp_mcu_id-gpio", 0);
	if (chip->warp_gpio.warp_mcu_id_gpio < 0) {
		...
		if (gpio_is_valid(chip->warp_gpio.warp_mcu_id_gpio)) {
			rc = gpio_request(chip->warp_gpio.warp_mcu_id_gpio,
					  "warp-mcu-id-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n",
					chip->warp_gpio.warp_mcu_id_gpio);
	...
	chip->warp_gpio.gpio_warp_mcu_id_default = pinctrl_lookup_state(
		chip->warp_gpio.pinctrl, "warp_mcu_id_default");
...
int oplus_warp_gpio_dt_init(struct oplus_warp_chip *chip)
{
	...
	chip->warp_gpio.switch1_ctr1_gpio =
		of_get_named_gpio(node, "qcom,charging_switch1_ctr1-gpio", 0);
	if (chip->warp_gpio.switch1_ctr1_gpio < 0) {
		...
		if (gpio_is_valid(chip->warp_gpio.switch1_ctr1_gpio)) {
			rc = gpio_request(chip->warp_gpio.switch1_ctr1_gpio,
					  "charging-switch1-ctr1-gpio");
			if (rc) {
				chg_err("unable to request gpio [%d]\n",
					chip->warp_gpio.switch1_ctr1_gpio);
			} else {
				gpio_direction_output(
					chip->warp_gpio.switch1_ctr1_gpio, 0);
			}
		}
	...
	if (1) {
		chip->warp_gpio.switch2_gpio = of_get_named_gpio(
			node, "qcom,charging_switch3-gpio", 0);
		...
		chip->warp_gpio.switch2_gpio = of_get_named_gpio(
			node, "qcom,charging_switch2-gpio", 0);
		if (chip->warp_gpio.switch2_gpio < 0) {
			chg_err("chip->warp_gpio.switch2_gpio not specified\n");
		} else {
			if (gpio_is_valid(chip->warp_gpio.switch2_gpio)) {
	...
	chip->warp_gpio.reset_gpio =
		of_get_named_gpio(node, "qcom,charging_reset-gpio", 0);
	...
	chip->warp_gpio.clock_gpio =
		of_get_named_gpio(node, "qcom,charging_clock-gpio", 0);
	if (chip->warp_gpio.clock_gpio < 0) {
		chg_err("chip->warp_gpio.reset_gpio not specified\n");
	} else {
		if (gpio_is_valid(chip->warp_gpio.clock_gpio)) {
			rc = gpio_request(chip->warp_gpio.clock_gpio,
					  "charging-clock-gpio");
			if (rc) {
...
void opchg_set_clock_active(struct oplus_warp_chip *chip)
{
	...
	pinctrl_select_state(chip->warp_gpio.pinctrl,
			     chip->warp_gpio.gpio_clock_sleep); /* PULL_down */
...
void opchg_set_clock_sleep(struct oplus_warp_chip *chip)
{
	...
	mutex_lock(&chip->pinctrl_mutex);
	pinctrl_select_state(chip->warp_gpio.pinctrl,
			     chip->warp_gpio.gpio_clock_active); /* PULL_up */
...
void opchg_set_data_active(struct oplus_warp_chip *chip)
{
	...
	pinctrl_select_state(chip->warp_gpio.pinctrl,
			     chip->warp_gpio.gpio_data_active); /* no_PULL */
	mutex_unlock(&chip->pinctrl_mutex);
}
...
void opchg_set_data_sleep(struct oplus_warp_chip *chip)
{
	mutex_lock(&chip->pinctrl_mutex);
	pinctrl_select_state(chip->warp_gpio.pinctrl,
			     chip->warp_gpio.gpio_data_sleep); /* PULL_down */
...
void opchg_set_reset_sleep(struct oplus_warp_chip *chip)
{
	if (chip->adapter_update_real == ADAPTER_FW_NEED_UPDATE ||
		...
		chg_debug(
			" adapter_fw_need_update:%d,btb_temp_over:%d,mcu_update_ing:%d,return\n",
...
void opchg_set_reset_active_force(struct oplus_warp_chip *chip)
{
	...
	mutex_lock(&chip->pinctrl_mutex);
	gpio_direction_output(chip->warp_gpio.switch1_gpio, 0); /* in 0*/
	gpio_direction_output(chip->warp_gpio.reset_gpio,
			      active_level); /* out 1 */
	...
	if (chip->dpdm_switch_mode == WARP_CHARGER_MODE) {
		gpio_direction_output(chip->warp_gpio.switch1_gpio,
				      1); /* in 1*/
	}
	mutex_unlock(&chip->pinctrl_mutex);
	chg_debug("%s\n", __func__);
...
static void delay_reset_mcu_work_func(struct work_struct *work)
{
	...
	struct oplus_warp_chip *chip = container_of(
		dwork, struct oplus_warp_chip, delay_reset_mcu_work);
...
{
	INIT_DELAYED_WORK(&chip->delay_reset_mcu_work,
			  delay_reset_mcu_work_func);
}
...
static bool is_allow_fast_chg_dummy(struct oplus_warp_chip *chip)
{
	...
	if (mcu_hwid_type == OPLUS_WARP_ASIC_HWID_RK826 ||
	    ...
	    is_warp_asic_hwid_check_by_i2c(chip)) {
#endif
		if (oplus_warp_get_fastchg_dummy_started() == true &&
			...
			chg_debug(
				" dummy_started true, allow_real true, reset adapter\n");
			return false;
		}
	}
...
void switch_fast_chg(struct oplus_warp_chip *chip)
{
	...
	if (is_allow_fast_chg_dummy(chip) == true) {
		if (oplus_warp_get_adapter_update_status() ==
			  ...
			  int device_type)
{
	...
	if (chip->w_soc_temp_to_mcu == false) {
		for (i = 1; i < reply_counts; i++) {
			gpio_set_value(chip->warp_gpio.data_gpio,
				       (ret_info >> (reply_counts - i - 1)) &
					       0x01);
			chg_err("send_bit[%d] = %d\n", i,
				(ret_info >> (reply_counts - i - 1)) & 0x01);
			opchg_set_clock_active(chip);
			usleep_range(1000, 1000);
			opchg_set_clock_sleep(chip);
			usleep_range(19000, 19000);
...
void opchg_set_switch_fast_charger(struct oplus_warp_chip *chip)
{
	...
	if (chip->warp_gpio.switch1_ctr1_gpio > 0) {
#ifndef OPLUS_CHG_OP_DEF
		gpio_direction_output(chip->warp_gpio.switch1_ctr1_gpio,
				      0); /* out 0*/
#else
		gpio_direction_output(chip->warp_gpio.switch1_ctr1_gpio,
				      1); /* out 1*/
...
#ifdef OPLUS_CHG_OP_DEF
		if (chg_chip && chg_chip->chg_ops->disconnect_pd &&
		    chg_chip->is_oplus_svid)
...
void oplus_warp_data_irq_init(struct oplus_warp_chip *chip)
{
	...
	node = of_find_compatible_node(NULL, NULL,
				       "mediatek, WARP_AP_DATA-eint");
	node_new = of_find_compatible_node(NULL, NULL,
					   "mediatek, WARP_EINT_NEW_FUNCTION");
	if (node) {
		if (node_new) {
			chip->warp_gpio.data_irq =
				gpio_to_irq(chip->warp_gpio.data_gpio);
...
void oplus_warp_eint_register(struct oplus_warp_chip *chip)
{
	...
	if (node) {
		...
		ret = request_irq(chip->warp_gpio.data_irq,
				  (irq_handler_t)irq_rx_handler,
				  IRQF_TRIGGER_RISING, "WARP_AP_DATA-eint",
				  chip);
		if (ret < 0) {
	...
	int retval = 0;
	opchg_set_data_active(chip);
	retval = request_irq(chip->warp_gpio.data_irq, irq_rx_handler,
			     IRQF_TRIGGER_RISING, "mcu_data", chip);
	if (retval < 0) {
