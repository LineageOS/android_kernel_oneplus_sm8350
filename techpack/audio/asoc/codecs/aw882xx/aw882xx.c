...
static int aw882xx_check_status_reg(struct aw882xx *aw882xx)
{
	...
	if (ret < 0) {
		...
		if ((AW882XX_STATUS_NORMAL_VALUE & AW882XX_STATUS_CHECK_MASK) != (reg_val & AW882XX_STATUS_CHECK_MASK)) {
			...
			if (PID_1852_ID == aw882xx->aw_pa->chip_id) {
	...
	if (ret) {
		...
		case RE_ERR_IV_ERR:
			...
			/*scnprintf(fd_buf, sizeof(fd_buf) - 1, \
					...
					aw882xx->index + 1, re, te);*/
			aw_dev_err(aw882xx->dev, "payload@@AW882xx SPK%u:IV data error(no tx or MEC not open), re=%d, te=%d", aw882xx->index + 1, re, te);
			break;
		case RE_ERR_OPEN:
			scnprintf(fd_buf, sizeof(fd_buf) - 1, \
...
static int aw882xx_codec_probe(aw_snd_soc_codec_t *aw_codec)
{
	...
	#ifdef OPLUS_FEATURE_SMARTPA_PM
	add_smartpa_pm_controls(aw882xx->codec);
	#endif // OPLUS_FEATURE_SMARTPA_PM
				...
				int num_dai)
{
	...
	unsigned int addr = aw882xx->i2c->addr;

	if ((dai_drv != NULL) && (num_dai > 0))
		for (i = 0; i < num_dai; i++) {
			snprintf(buf, 49, "%s-%x-%x", dai_drv[i].name, i2cbus,
				addr);
			dai_drv[i].name = aw882xx_devm_kstrdup(aw882xx->dev, buf);

			snprintf(buf, 49, "%s-%x-%x",
				...
				const char *buf, size_t count)
{
	...
	aw_dev_info(aw882xx->dev, "set g_print_dbg : [%u]", g_print_dbg);
