...
static int aw_cali_svc_dev_get_re_noram(struct aw_device *aw_dev)
{
	int32_t re[AW_CALI_READ_TIMES];
	int ret, i;
	int32_t sum = 0;
...
static int aw_cali_svc_dev_cali_re_nonvram(struct aw_device *aw_dev, unsigned int flag)
{
	int ret;
	...
	/*dev get echo cali re*/
	ret = aw_cali_svc_dev_get_re_noram(aw_dev);
	if (ret < 0)
		goto exit;
exit:
	aw_cali_svc_dev_cali_mode_en(aw_dev, CALI_TYPE_RE, false, flag);
...
static int aw_cali_svc_devs_cali_re_nonvram(struct aw_device *aw_dev, unsigned int flag)
{
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		ret = aw_cali_svc_dev_get_re_noram(local_dev);
		if (ret < 0)
			goto exit;
	}
...
static int aw_cali_svc_cali_re_nonvram(struct aw_device *aw_dev, bool is_single, unsigned int flag)
{
	if (is_single)
		return aw_cali_svc_dev_cali_re_nonvram(aw_dev, flag);
	else
		return aw_cali_svc_devs_cali_re_nonvram(aw_dev, flag);
	return 0;
}
...
int aw_cali_svc_cali_cmd(struct aw_device *aw_dev, int cali_cmd, bool is_single, unsigned int flag)
{
	...
	case AW_CALI_CMD_RE_NONVRAM:
		return aw_cali_svc_cali_re_nonvram(aw_dev, is_single, flag);
	default:
...
static ssize_t aw_cali_attr_re_show_nonvram(struct device *dev,
	struct device_attribute *attr, char *buf)//add by wan
{
	int ret, i;
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	struct aw_device *aw_dev = aw882xx->aw_pa;
	...
	ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE_NONVRAM, is_single_cali, CALI_OPS_HMUTE);
	if (ret < 0) {
		len += snprintf(buf+len, PAGE_SIZE-len, "cali Re cmd failed\n");
		return len;
	}
...
int aw882xx_load_re_for_test(int32_t chan, int32_t *re)
{
    ...
    ret = aw_cali_read_re_from_nvram(&cali_re, chan);
    if (ret < 0) {
...
static ssize_t aw_origin_re_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	ssize_t len = 0;
	...
	if (is_single_cali) {
		aw882xx_load_re_for_test(0, &re[0]);
		len += snprintf(buf+len,
			PAGE_SIZE - len, "left=%d\n", re[0]);
	} else {
		aw882xx_load_re_for_test(0, &re[0]);
		aw882xx_load_re_for_test(1, &re[1]);
		len += snprintf(buf+len,
...
static ssize_t aw_range_re_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    struct aw882xx *aw882xx = dev_get_drvdata(dev);
    ...
    if (is_single_cali) {
        len += snprintf(buf+len,
			PAGE_SIZE - len, "left[%u, %u]\n",
			range_buf[RE_MIN_FLAG], range_buf[RE_MAX_FLAG]);
    } else {
        len += snprintf(buf+len,
			PAGE_SIZE - len, "left[%u, %u], right[%u, %u]\n",
			range_buf[RE_MIN_FLAG], range_buf[RE_MAX_FLAG], range_buf[RE_MIN_FLAG], range_buf[RE_MAX_FLAG]);
    }
	...
	struct device_attribute *attr, char *buf)
{
	...
	aw_cali_svc_get_dev_re_range(aw_dev, range_buf);
	len += snprintf(buf + len, PAGE_SIZE - len, "left[%u, %u]\n", range_buf[RE_MIN_FLAG], range_buf[RE_MAX_FLAG]);
...
static DEVICE_ATTR(calibration, S_IRUGO,
	aw_cali_attr_re_show_nonvram, NULL);
					...
					struct class_attribute *attr, const char *buf, size_t len)
{
	...
	if (time < 400) {
		pr_err("[Awinic] %s:time:%u is too short, no set\n",
			__func__, time);
		return -EINVAL;
	}
...
static ssize_t aw_class_re_range_show(struct  class *class, struct class_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ret; i++) {
		len += snprintf(buf+len, PAGE_SIZE-len, "%s:re_min:%u mOhms re_max:%u mOhms ",
			...
			unsigned int cmd, unsigned long arg)
{
	...
	if (((_IOC_TYPE(cmd)) != (AW_IOCTL_MAGIC))) {
		pr_err("%s: cmd magic err", __func__);
		return -EINVAL;
	}
...
int aw_cali_init(struct aw_cali_desc *cali_desc)
{
	...
	aw_dev_err(aw_dev->dev, "device %d",dev_num);
	if (dev_num >= 1) {
		is_single_cali = false;
	} else {
		is_single_cali = true;
	}
