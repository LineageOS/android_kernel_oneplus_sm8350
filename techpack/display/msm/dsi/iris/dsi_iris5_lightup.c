...
//iris_alloc_seq_space(), seq = 293, lut = 2822
#define IRIS_SEQ_LUT_CMDS_LEN 3500
#define IRIS_EXT_CLK
...
void iris_init(struct dsi_display *display, struct dsi_panel *panel)
{
	...
	if (!iris_virtual_display(display) && pcfg->valid >= PARAM_PARSED) {
		...
		pcfg->cur_vres_in_iris = FHD_H_2412;
...
static void _iris_reset_out_cmds(void)
{
	...
	IRIS_LOGI("%s(), seq = %u, lut = %u, sum = %d", __func__,
			pcfg->dtsi_cmds_cnt, pcfg->lut_cmds_cnt, sum);

	memset(pcfg->iris_cmds.iris_cmds_buf, 0x00, IRIS_SEQ_LUT_CMDS_LEN * sizeof(struct dsi_cmd_desc));
...
void iris_alloc_seq_space(void)
{
	...
	sum = IRIS_SEQ_LUT_CMDS_LEN * sizeof(struct dsi_cmd_desc);
	pdesc = vmalloc(sum);
	if (!pdesc) {
	...
	IRIS_LOGI("%s(), alloc count= %d, p = %p", __func__, sum , pcfg->iris_cmds.iris_cmds_buf);
	_iris_reset_out_cmds();
...
static void _iris_pre_lightup(struct dsi_panel *panel)
{
	...
	int len = 0;
	bool high = false;
	bool freq_90hz = false;

	if ((panel->cur_mode->timing.refresh_rate == HIGH_FREQ)
			&& (pcfg->panel->cur_mode->timing.v_active == FHD_H
			|| pcfg->panel->cur_mode->timing.v_active == FHD_H_2412))
		high = true;
#ifdef IRIS_SUPPORT_90HZ
	if (panel->cur_mode->timing.refresh_rate == FREQ_90HZ)
		freq_90hz = true;
#endif
	iris_init_ipopt_ip(popt, IP_OPT_MAX);
	//sys pll
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_SYS, freq_90hz ? 0xA2 : (high ? 0xA1:0xA0), 0x1);
	//dtg
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_DTG, freq_90hz ? 0x02 : (high ? 0x1:0x0), 0x1);
	//mipi tx
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_TX, freq_90hz ? 0x05 : (high ? 0x4:0x0), 0x1);
	//mipi abp
	len = iris_update_ip_opt(popt, IP_OPT_MAX, IRIS_IP_RX, freq_90hz ? 0xE2 : (high ? 0xE1:0xE0), 0x0);
	_iris_update_pq_seq(popt, len);

	/*send rx cmds first with low power*/
	iris_send_ipopt_cmds(IRIS_IP_RX, freq_90hz ? 0xF3 : (high ? 0xF2 : 0xF1));
...
int iris_enable(struct dsi_panel *panel, struct dsi_panel_cmd_set *on_cmds)
{
	...
	if (panel->cur_mode->timing.refresh_rate == HIGH_FREQ
			&& ((panel->cur_mode->timing.v_active == QHD_H)
				|| (panel->cur_mode->timing.v_active == QHD_H_3216))) {
