...
bool iris_fast_cmd_abyp_exit(void)
{
	...
	if ((next_fps == HIGH_FREQ) && (next_vres == FHD_H || next_vres == FHD_H_2412))
		high = true;
	else
		high = false;

#ifdef IRIS_SUPPORT_90HZ
	if (next_fps == FREQ_90HZ)
		freq_90hz = true;
#endif
	if (debug_lp_opt & 0x400)
		ktime0 = ktime_get();
...
exit_abyp_loop:
	...
	if (pcfg->iris_initialized) {
		if ((pcfg->cur_fps_in_iris == next_fps) && (pcfg->cur_vres_in_iris == next_vres)) {
			...
			iris_send_ipopt_cmds(IRIS_IP_SYS, freq_90hz? 0x29 : (high ? 0x28:0x27));
			udelay(100);
			//switch clock mux & power on domains
			iris_send_ipopt_cmds(IRIS_IP_SYS, 0x21);
	...
	if (abyp_status_gpio == 0) {
		if (pcfg->iris_initialized == false) {
			...
			if (pcfg->cur_vres_in_iris != next_vres) {
				...
				//only fps change
				iris_send_ipopt_cmds(IRIS_IP_RX, freq_90hz? 0xE2 : (high ? 0xE1:0xE0));
