...
int ois_download_fw_thread(void *arg)
{
	...
	if (o_ctrl->cam_ois_state != CAM_OIS_CONFIG) {
		...
		if (o_ctrl->ois_power_state == CAM_OIS_POWER_OFF){
			...
			CAM_ERR(CAM_OIS,
				"ois type=%d, OIS already power on, no need to power on again",
				o_ctrl->ois_type);
		}
	...
	RamWrite32A_oplus(o_ctrl, 0xf012, 0x0);
	RamWrite32A_oplus(o_ctrl, 0xf010, 0x0);
	mutex_unlock(&(o_ctrl->do_ioctl_ois));
	return rc;
}
#endif
	...
	unsigned int cmd, void *arg)
{
	...
	case VIDIOC_CAM_SENSOR_STATR:
		if (arg) {
			if (copy_from_user(&gyro_vendor, (void  __user*)(arg),
					sizeof(gyro_vendor))) {
		...
		if (o_ctrl->ois_power_state == CAM_OIS_POWER_ON) {
			CAM_INFO(CAM_OIS,
				"do not need to create ois download fw thread");
			o_ctrl->ois_power_down_thread_exit = true;
			mutex_unlock(&(o_ctrl->ois_power_down_mutex));
			o_ctrl->cam_ois_state = CAM_OIS_CONFIG;
			mutex_unlock(&(o_ctrl->ois_mutex));
			...
			o_ctrl->ois_downloadfw_thread = kthread_run(
					ois_download_fw_thread, o_ctrl,
					o_ctrl->ois_name);
			if (!o_ctrl->ois_downloadfw_thread) {
				CAM_ERR(CAM_OIS,
					"create ois download fw thread failed");
				mutex_unlock(&(o_ctrl->ois_power_down_mutex));
				mutex_unlock(&(o_ctrl->ois_mutex));
				return -1;
			}
	...
	struct device *master_dev, void *data)
{
	...
	if (kfifo_alloc(&o_ctrl->ois_hall_data_fifo,
			SAMPLE_COUNT_IN_DRIVER * SAMPLE_SIZE_IN_DRIVER,
			GFP_KERNEL)) {
		CAM_ERR(CAM_OIS, "failed to init ois_hall_data_fifo");
	}

	if (kfifo_alloc(&o_ctrl->ois_hall_data_fifoV2,
			SAMPLE_COUNT_IN_DRIVER * SAMPLE_SIZE_IN_DRIVER,
			GFP_KERNEL)) {
