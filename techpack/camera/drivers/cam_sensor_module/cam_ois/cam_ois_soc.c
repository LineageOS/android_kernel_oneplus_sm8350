...
int cam_ois_driver_soc_init(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get ois_gyro,position failed rc:%d, set default value to %d",
				ret, o_ctrl->ois_gyro_position);
	} else {
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get ois,type failed rc:%d, default %d",
				ret, o_ctrl->ois_type);
	} else {
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get ois_gyro,type failed rc:%d, default %d",
				ret, o_ctrl->ois_gyro_vendor);
	} else {
		o_ctrl->ois_gyro_vendor = (uint8_t)id;
	}

	ret = of_property_read_string_index(of_node, "ois,name", 0,
			(const char **)&p);
	if (ret) {
		CAM_ERR(CAM_OIS, "get ois,name failed rc:%d, set default value to %s",
				ret, o_ctrl->ois_name);
	} else {
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get ois_module,vendor failed rc:%d, default %d",
				ret, o_ctrl->ois_module_vendor);
	} else {
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get ois_actuator,vednor failed rc:%d, default %d",
				ret, o_ctrl->ois_actuator_vendor);
	} else {
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get ois,fw failed rc:%d, default %d",
				ret, o_ctrl->ois_fw_flag);
	} else {
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get change_cci failed rc:%d, default %d",
				ret, o_ctrl->ois_change_cci);
	} else {
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get ois_eis_function failed rc:%d, default %d",
				ret, o_ctrl->ois_eis_function);
	} else {
	...
	if (ret) {
		...
		CAM_ERR(CAM_OIS, "get download,fw failed rc:%d, default %d",
				ret, o_ctrl->cam_ois_download_fw_in_advance);
	} else {
